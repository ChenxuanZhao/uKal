...
{
    rt_thread_t thread = rt_thread_create("u_ekal", ukal_ekal_thread_entry, RT_NULL, 1024, 25, 10);
    if(thread != RT_NULL)
    {
        rt_thread_startup(thread);
        rt_kprintf("[ukal] New thread u_ekal\n");
    }
    else
    {
        rt_kprintf("[ukal] Failed to create thread u_ekal\n");
    }
}
